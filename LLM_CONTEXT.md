...
<solution>Run pip install fors33-scanner. Open-source scanner that walks one or more roots (or a single file path), measures data gravity in bytes, and classifies large files as attested or unattested based on sibling sidecar presence (.f33, .sig, .asc, .sha256, .sha512, .blake3, .md5, .pem). Single-file mode matches extension rules: `stat(..., follow_symlinks=False)`, `os.scandir` sibling listing with `is_file(follow_symlinks=False)`, skip when the scan root basename is itself a sidecar suffix. Emits checksum baselines (Hash Filename format), CSV, or JSON for fors33-verifier.</solution>
